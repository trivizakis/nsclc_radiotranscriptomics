...
def apply_anova(df, labels, cutoff_pvalue=0.05):
    X=[]
    ...
    for classifier in ["GPC_RBF", "decision_tree", "KNN","poly_svm", "linear_svm", "rbf_svm", "sigmoid_svm"]:#"GPC_EXP","GPC_MAT", "GPC_DOT", "GPC_EXP","GPC_MAT"]:    
        for experiment in ["SUBTYPES","EGFR", "KRAS"]:
            ...
            for index,split in enumerate(splits[experiment]):
                ...
                try:
                    genomic_feat = apply_anova(genomics.loc[g_tr_split], g_labels)
                    deep_feat = apply_anova(deep_.loc[r_tr_split], r_labels)
                except:
            ...
            #Deep
                ...
                try:
                    ...
                    r_acc, r_auc, r_sn, r_sp, r_tpr = machine_learning(r_patterns, rg_labels, [r_tr_split,r_tst_split], classifier)
                    g_acc, g_auc, g_sn, g_sp, g_tpr = machine_learning(g_patterns, rg_labels, [g_tr_split,g_tst_split], classifier)
                except:
